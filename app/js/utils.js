...
const Utils = {
  ...
  getStatusBadge(status, intervention) {
    // For PMC interventions with cumulative progress, show a descriptive badge
    if (intervention?.pmcProgress && status === 'completed') {
      return `<span class="badge badge-completed" title="${intervention.pmcProgress}">${intervention.pmcProgress}</span>`;
    }
    ...
    return list.filter(i => {
      ...
      if (filters.search) {
        ...
        const searchText = [
          ...
          machine?.jobNumber,
          Utils.getTechnicianNames(i), i.type, i.status, i.priority
        ].join(' ').toLowerCase();
  ...
  // Returns the canonical array of technician IDs for an intervention,
  // supporting both the legacy single-id field and the new multi-id array.
  getTechIds(intervention) {
    if (!intervention) return [];
    if (Array.isArray(intervention.technicianIds) && intervention.technicianIds.length > 0) {
      return intervention.technicianIds;
    }
    return intervention.technicianId ? [intervention.technicianId] : [];
  },

  // Returns a display string of all assigned technician names.
  getTechnicianNames(intervention) {
    const ids = this.getTechIds(intervention);
