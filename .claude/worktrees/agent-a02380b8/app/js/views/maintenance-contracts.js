...
Views.MaintenanceContracts = {
  ...
  /* ── PLAN VISIT MODAL ───────────────────────────────────── */
  _openPlanVisitModal(contractId, visitIndex) {
    const contract = Storage.getMaintenanceContractById(contractId);
    if (!contract) return;
    ...
    const existing = Storage.getInterventions().find(i =>
      i.type === 'pmc' &&
  ...
  _submitPlanVisit(contractId, visitIndex, existingId) {
    ...
    if (existingId) {
      ...
      Storage.updateIntervention(existingId, {
        scheduledDate,
