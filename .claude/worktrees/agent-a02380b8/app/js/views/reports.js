...
Views.Reports = {
  ...
  _renderAll() {
    ...
    container.innerHTML = `
      ...
      ${this._pmcSection(interventions)}
  ...
  _techPerformanceSection(interventions) {
    ...
    const rows = technicians.map(tech => {
      const assigned  = interventions.filter(i => Utils.getTechIds(i).includes(tech.id));
  ...
  _cpThHTML(col, label, extraStyle) {
    const isActive = this._cpSortCol === col;
    const arrow = isActive ? (this._cpSortDir === 'asc' ? ' ▲' : ' ▼') : '';
    const base = `cursor:pointer;user-select:none;white-space:nowrap${isActive ? ';color:var(--blue)' : ''}`;
    return `<th style="${base}${extraStyle ? ';' + extraStyle : ''}" onclick="Views.Reports._sortContractProgress('${col}')">${label}${arrow}</th>`;
  },

  _sortContractProgress(col) {
    if (this._cpSortCol === col) {
      this._cpSortDir = this._cpSortDir === 'asc' ? 'desc' : 'asc';
    } else {
      this._cpSortCol = col;
      this._cpSortDir = 'asc';
    }

    const contracts = Storage.getMaintenanceContracts ? Storage.getMaintenanceContracts() : [];
    const clients   = appState.clients;
    const machines  = appState.machines;
    const today     = new Date();

    const filtered = contracts.filter(c => {
      if (!this._dateFrom) return true;
    ...
    const getValue = (c) => {
      ...
      const completed = c.completedVisits || [];
      const remaining = schedule.filter(d => !completed.includes(d));
      const overdue   = remaining.filter(d => new Date(d) < today).length;
      const nextVisit = remaining.find(d => new Date(d) >= today) || null;
      const diffDays  = Math.ceil((new Date(c.endDate) - today) / 86400000);
      const statusOrder = diffDays < 0 ? 2 : diffDays <= 30 ? 1 : 0;
      switch (col) {
        case 'client':    return (client?.name || '').toLowerCase();
        case 'machine':   return (machine?.model || '').toLowerCase();
        case 'freq':      return c.visitsPerYear || 0;
        case 'progress':  return schedule.length > 0 ? completed.length / schedule.length : 0;
        case 'overdue':   return overdue;
        case 'next':      return nextVisit || 'zzz';
        case 'status':    return statusOrder;
    ...
    const sorted = [...filtered].sort((a, b) => {
      ...
      return this._cpSortDir === 'desc' ? -cmp : cmp;
    });
    ...
    tbody.innerHTML = sorted.map(c => {
      ...
      const completed = c.completedVisits || [];
      const remaining = schedule.filter(d => !completed.includes(d));
      const overdue   = remaining.filter(d => new Date(d) < today);
      const nextVisit = remaining.find(d => new Date(d) >= today) || null;
      const diffDays  = Math.ceil((new Date(c.endDate) - today) / 86400000);
      let statusBadge;
      if (diffDays < 0) {
        statusBadge = `<span style="padding:2px 8px;border-radius:10px;font-size:0.786rem;font-weight:600;background:#FEE2E2;color:#991B1B">Expired</span>`;
      } else if (diffDays <= 30) {
        statusBadge = `<span style="padding:2px 8px;border-radius:10px;font-size:0.786rem;font-weight:600;background:#FEF9C3;color:#854D0E">Expiring Soon</span>`;
      } else {
        statusBadge = `<span style="padding:2px 8px;border-radius:10px;font-size:0.786rem;font-weight:600;background:#DCFCE7;color:#166534">Active</span>`;
      }
      const progPct   = schedule.length > 0 ? Math.round((completed.length / schedule.length) * 100) : 0;
      const progColor = progPct === 100 ? '#059669' : progPct >= 50 ? '#D97706' : '#3B82F6';
      return `<tr>
        <td><div style="font-weight:500">${Utils.escapeHtml(client?.name || '—')}</div><div style="font-size:0.75rem;color:var(--gray-400)">${Utils.escapeHtml(client?.region || '')}</div></td>
        <td><div>${Utils.escapeHtml(machine?.model || '—')}</div><div style="font-family:monospace;font-size:0.75rem;color:var(--gray-400)">${Utils.escapeHtml(machine?.jobNumber || '')}</div></td>
        ...
        <td>
          <div style="display:flex;align-items:center;gap:8px">
            ...
            <span style="font-size:0.8rem;white-space:nowrap">${completed.length} / ${schedule.length}</span>
          </div>
        </td>
        <td style="text-align:center;color:${overdue.length > 0 ? 'var(--red)' : 'var(--gray-400)'};font-weight:${overdue.length > 0 ? '600' : '400'}">${overdue.length > 0 ? overdue.length : '—'}</td>
        <td style="font-size:0.857rem">${nextVisit ? Utils.formatDate(nextVisit) : '<span style="color:var(--gray-400)">—</span>'}</td>
        <td>${statusBadge}</td>
        <td style="font-size:0.857rem">${Utils.formatDate(c.endDate)}</td>
      </tr>`;
    }).join('') || `<tr><td colspan="8"><div class="table-empty"><p class="table-empty-text">No contracts in selected period</p></div></td></tr>`;

    // Update header indicators
    const thead = document.getElementById('cpTheadRow');
    if (thead) {
      thead.innerHTML = `
        ${this._cpThHTML('client',   'Client')}
        ${this._cpThHTML('machine',  'Machine')}
        ${this._cpThHTML('freq',     'Frequency', 'text-align:center')}
        ${this._cpThHTML('progress', 'Visit Progress')}
        ${this._cpThHTML('overdue',  'Overdue', 'text-align:center')}
        ${this._cpThHTML('next',     'Next Visit')}
        ${this._cpThHTML('status',   'Status')}
        ${this._cpThHTML('end',      'End Date')}
      `;
    }
  },

  _pmcSection(interventions) {
    const contracts   = Storage.getMaintenanceContracts ? Storage.getMaintenanceContracts() : [];
    const clients     = appState.clients;
    const machines    = appState.machines;
    const technicians = appState.users.filter(u => u.role === 'technician');
    ...
    // Show all contracts — PMC contracts span a full year so they are not
    // filtered by the report date range (which is designed for interventions).
    // Only exclude contracts that fully expired before the selected From date.
    const filtered = contracts.filter(c => {
      if (!this._dateFrom) return true;
      const end  = new Date(c.endDate + 'T23:59:59');
      const from = new Date(this._dateFrom);
    ...
    const activeContracts = filtered.filter(c => new Date(c.endDate) >= today).length;
    const expiredContracts = filtered.filter(c => new Date(c.endDate) < today).length;
    const expiringSoon    = filtered.filter(c => {
      const diff = (new Date(c.endDate) - today) / 86400000;
      return diff >= 0 && diff <= 30;
    }).length;

    let totalPlanned = 0, totalCompleted = 0, totalOverdue = 0;
    filtered.forEach(c => {
      const schedule  = c.schedule || [];
      const completed = c.completedVisits || [];
      totalPlanned   += schedule.length;
      totalCompleted += completed.length;
      totalOverdue   += schedule.filter(d => !completed.includes(d) && new Date(d) < today).length;
    });
    const totalRemaining = totalPlanned - totalCompleted;
    const visitRate = Utils.percent(totalCompleted, totalPlanned);

    // PMC interventions in scope
    const pmcInterventions = interventions.filter(i => i.type === 'pmc');
    ...
    const contractRows = filtered.map(c => {
      ...
      return `
        <tr>
          <td>
            <div style="font-weight:500">${Utils.escapeHtml(client?.name || '—')}</div>
            <div style="font-size:0.75rem;color:var(--gray-400)">${Utils.escapeHtml(client?.region || '')}</div>
          </td>
          <td>
            <div>${Utils.escapeHtml(machine?.model || '—')}</div>
            <div style="font-family:monospace;font-size:0.75rem;color:var(--gray-400)">${Utils.escapeHtml(machine?.jobNumber || '')}</div>
          </td>
          <td style="text-align:center">${c.visitsPerYear}×/yr</td>
          <td>
            <div style="display:flex;align-items:center;gap:8px">
    ...
    // Technician PMC workload rows
    const techRows = technicians.map(tech => {
      const assigned  = pmcInterventions.filter(i => Utils.getTechIds(i).includes(tech.id));
      const completed = assigned.filter(i => i.status === 'completed');
      const active    = assigned.filter(i => CONFIG.OPEN_STATUSES.includes(i.status));
      ...
      return `
        <tr>
          <td>
            <div style="display:flex;align-items:center;gap:8px">
              <div style="width:28px;height:28px;border-radius:50%;background:var(--blue);color:white;display:flex;align-items:center;justify-content:center;font-size:0.714rem;font-weight:600;flex-shrink:0">${Utils.getInitials(tech.name)}</div>
              <div>
                <div class="font-semibold">${Utils.escapeHtml(tech.name)}</div>
                <div class="text-xs text-muted">${Utils.escapeHtml(tech.email)}</div>
              </div>
          ...
          <td style="color:var(--green)">${completed.length}</td>
          <td style="color:var(--orange)">${active.length}</td>
          <td>${Utils.percent(completed.length, assigned.length)}%</td>
        </tr>
      `;
    }).filter(Boolean).join('');
    ...
    return `
      <div class="report-section">
        <div class="report-section-title">PMC – Preventive Maintenance Contracts</div>

        <!-- KPI row -->
        <div class="stat-row" style="margin-bottom:20px">
          <div class="stat-box">
            <div class="stat-box-value">${totalContracts}</div>
            <div class="stat-box-label">Total Contracts</div>
          </div>
          <div class="stat-box">
            <div class="stat-box-value" style="color:var(--green)">${activeContracts}</div>
            <div class="stat-box-label">Active</div>
          </div>
          <div class="stat-box">
            <div class="stat-box-value" style="color:var(--yellow)">${expiringSoon}</div>
            <div class="stat-box-label">Expiring Soon</div>
          </div>
          <div class="stat-box">
            <div class="stat-box-value" style="color:var(--gray-400)">${expiredContracts}</div>
          ...
          <div class="stat-box">
            <div class="stat-box-value" style="color:var(--orange)">${totalRemaining}</div>
            <div class="stat-box-label">Visits Remaining</div>
          </div>
          <div class="stat-box">
            <div class="stat-box-value" style="color:${totalOverdue > 0 ? 'var(--red)' : 'var(--gray-400)'}">${totalOverdue}</div>
            <div class="stat-box-label">Overdue Visits</div>
          </div>
          <div class="stat-box">
            <div class="stat-box-value" style="color:var(--blue)">${visitRate}%</div>
            <div class="stat-box-label">Visit Completion Rate</div>
          </div>
        </div>

        <!-- Contract detail table -->
        <div class="card" style="margin-bottom:20px">
          <div class="card-header"><span class="card-title">Contract Progress</span></div>
            ...
            <div class="table-wrapper">
              <table class="data-table">
                ...
                <tbody id="cpTableBody">
                  ${contractRows || `<tr><td colspan="8"><div class="table-empty"><p class="table-empty-text">No contracts in selected period</p></div></td></tr>`}
                </tbody>
              </table>
            </div>
          </div>
        ...
        <!-- Technician PMC workload -->
        ${techRows ? `
        <div class="card">
          <div class="card-header"><span class="card-title">Technician PMC Workload</span></div>
          <div class="card-body" style="padding:0">
            <div class="table-wrapper">
              <table class="data-table">
                <thead>
